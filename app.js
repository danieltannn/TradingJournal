...
async function fetchPricesJson() {
  ...
  if (!res.ok) throw new Error('prices.json not found — tap Refresh to fetch prices');
  const meta = await res.json();
  return JSON.parse(atob(meta.content.replace(/\n/g, '')));
}

function applyPriceData(data, openPositions) {
  ...
  for (const sym of CURRENT_TICKERS) {
    ...
    if (card) {
      const meta2 = card.querySelector('.inv-sym-meta');
      if (meta2) {
        let badge = meta2.querySelector('.live-price');
        if (!badge) { badge = document.createElement('span'); badge.className = 'live-price badge open'; badge.style.cssText = 'font-size:10px;padding:2px 5px;margin-left:4px'; meta2.appendChild(badge); }
  ...
  const holdUnrl    = el('hold-unreal');
  const holdReal    = el('hold-real');
  ...
  // Update SGD portfolio using live rate
  if (usdsgd > 0) {
...
async function fetchAndUpdateLivePrices(tickers, openPositions) {
  try {
    const data = await fetchPricesJson();
    applyPriceData(data, openPositions);
  } catch(e) {
    // silent fail — shows CSV prices
  }
}
